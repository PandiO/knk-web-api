    ...
    public class UserMappingProfile : Profile
    {
        public UserMappingProfile()
        {
            ...
            CreateMap<UserDto, User>()
                ...
                .ForMember(dest => dest.Uuid, src => src.MapFrom(src => src.Uuid))
                .ForMember(dest => dest.Email, src => src.MapFrom(src => src.Email))
                // Balances, XP and the personal reward multipliers are never written by the
                // generic edit (KNG-22, currency DESIGN.md §1.4 A1/A8): they change only through
                // PUT /api/users/{id}/balances (locked, capped, node-checked, audited) and
                // PUT /api/users/{id}/multipliers. A form that still sends them is ignored.
                .ForMember(dest => dest.Coins, opt => opt.Ignore())
                .ForMember(dest => dest.Gems, opt => opt.Ignore())
                .ForMember(dest => dest.ExperiencePoints, opt => opt.Ignore())
                .ForMember(dest => dest.PersonalSalaryMultiplier, opt => opt.Ignore())
                .ForMember(dest => dest.PersonalGemBonusMultiplier, opt => opt.Ignore())
                .ForMember(dest => dest.PersonalExpBonusMultiplier, opt => opt.Ignore())
                .ForMember(dest => dest.EmailVerified, src => src.MapFrom(src => src.EmailVerified))
                .ForMember(dest => dest.AccountCreatedVia, src => src.MapFrom(src => src.AccountCreatedVia))
                .ForMember(dest => dest.GatePassThroughMethodDefault, src => src.MapFrom(src => src.GatePassThroughMethodDefault))
